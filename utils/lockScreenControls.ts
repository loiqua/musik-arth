...
const setupMediaSession = async (
  ...
  // Vérifier si nous sommes sur Android et si l'API MediaSession est disponible
  if (Platform.OS !== 'android') {
    return;
  }
  
  try {
    // Utilisation de l'API MediaSession de façon sécurisée
...
export const setupNotificationActionHandlers = (
  ...
  // Supprimer les écouteurs précédents s'ils existent
  const notificationModule = Notifications as any;
  if (notificationModule.removeAllNotificationListeners) {
    notificationModule.removeAllNotificationListeners();
  }
  
  // Ajouter un nouvel écouteur pour les réponses aux notifications
